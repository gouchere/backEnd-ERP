...
@Service
public class JDBCArticlePackRepo implements JDBCEntityRepo<YvsBaseArticlePack, Long> {

	@Autowired
	CustomJdbcRepo<YvsBaseArticlePack> jdbcRepository;

	public JDBCArticlePackRepo() {
	...
	public List<YvsBaseArticlePack> findAll(){
		...
		return jdbcRepository.loadData(YvsBaseArticlePack.class, query, new SqlParams[] {});
	}

	public YvsBaseArticlePack findById(Long id){
		String query="SELECT y.* FROM yvs_base_articles y WHERE id=:id";
		return jdbcRepository.loadOneData(YvsBaseArticlePack.class, query,new SqlParams[] {new SqlParams("id", id)});
	}
}
