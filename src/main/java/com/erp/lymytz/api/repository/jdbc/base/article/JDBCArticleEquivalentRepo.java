...
@Service
public class JDBCArticleEquivalentRepo implements JDBCEntityRepo<YvsBaseArticleEquivalent, Long> {

	@Autowired
	CustomJdbcRepo<YvsBaseArticleEquivalent> jdbcRepository;

	public JDBCArticleEquivalentRepo() {
	...
	public List<YvsBaseArticleEquivalent> findAll(){
		...
		return jdbcRepository.loadData(YvsBaseArticleEquivalent.class, query, new SqlParams[] {});
	}

	public YvsBaseArticleEquivalent findById(Long id){
		String query="SELECT y.* FROM yvs_base_articles y WHERE id=:id";
		return jdbcRepository.loadOneData(YvsBaseArticleEquivalent.class, query,new SqlParams[] {new SqlParams("id", id)});
	}
}
