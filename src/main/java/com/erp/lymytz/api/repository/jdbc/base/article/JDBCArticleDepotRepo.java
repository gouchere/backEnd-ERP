...
@Service
public class JDBCArticleDepotRepo implements JDBCEntityRepo<YvsBaseArticleDepot, Long> {

	@Autowired
	CustomJdbcRepo<YvsBaseArticleDepot> jdbcRepository;

	public JDBCArticleDepotRepo() {
	...
	public List<YvsBaseArticleDepot> findAll(){
		...
		return jdbcRepository.loadData(YvsBaseArticleDepot.class, query, new SqlParams[] {});
	}

	public YvsBaseArticleDepot findById(Long id){
		String query="SELECT y.* FROM yvs_base_articles y WHERE id=:id";
		return jdbcRepository.loadOneData(YvsBaseArticleDepot.class, query,new SqlParams[] {new SqlParams("id", id)});
	}
}
