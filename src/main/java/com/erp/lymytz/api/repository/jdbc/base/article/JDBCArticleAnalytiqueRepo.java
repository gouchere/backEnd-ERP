...
@Service
public class JDBCArticleAnalytiqueRepo implements JDBCEntityRepo<YvsBaseArticleAnalytique, Long> {

	@Autowired
	CustomJdbcRepo<YvsBaseArticleAnalytique> jdbcRepository;

	public JDBCArticleAnalytiqueRepo() {
	...
	public List<YvsBaseArticleAnalytique> findAll(){
		...
		return jdbcRepository.loadData(YvsBaseArticleAnalytique.class, query, new SqlParams[] {});
	}

	public YvsBaseArticleAnalytique findById(Long id){
		String query="SELECT y.* FROM yvs_base_articles y WHERE id=:id";
		return jdbcRepository.loadOneData(YvsBaseArticleAnalytique.class, query,new SqlParams[] {new SqlParams("id", id)});
	}
}
